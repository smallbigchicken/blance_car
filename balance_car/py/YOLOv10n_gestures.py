...
class YOLOv10ONNX:
    ...
    def __init__(
        ...
        # 图片短边/长边缩放到 imgsz 的输入尺寸
        self.imgsz = imgsz
        # 根据 use_gpu 参数选择 ONNX 运行时的执行提供器（示例使用 CANNExecutionProvider）
        providers = ["CANNExecutionProvider"] if use_gpu else ["CPUExecutionProvider"]
        self.session = ort.InferenceSession(onnx_path, providers=providers)
        # 输入输出名称，用于 session.run 调用
    ...
    def infer(self, img, conf_thres=0.25, iou_thres=0.45, save_path=None):
        ...
        boxes, scores, classes = self.postprocess(
            pred, r, dw, dh, conf_thres, iou_thres
        )
        # 按置信度从大到小排序
    ...
    def detect(self, img, conf_thres=0.25, iou_thres=0.45, save_path=None):
        """
        对单张图像进行手势检测并返回业务编号：
        - 若无检测或类别映射为 -1 则返回 -1（代表无有效手势）
        """
        box, score, classes, img_drawn = self.infer(
            img, conf_thres, iou_thres, save_path
        )
        if classes is None:
            return -1
