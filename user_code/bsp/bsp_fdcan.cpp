...
void can_filter_init(void)
{
    ...
    fdcan_filter.FilterType = FDCAN_FILTER_RANGE;                  //允许接收双ID                 
...
{	
  ...
  TxHeader.DataLength = get_fdcan_dlc(len);																		// 发送数据长度 
...
uint32_t get_fdcan_dlc(uint32_t len)
{
    switch(len) {
        ...
        // 如果是 CAN-FD 还可以支持 12, 16, 20... 64
        default: return FDCAN_DLC_BYTES_8; // 默认防错
    }
}
